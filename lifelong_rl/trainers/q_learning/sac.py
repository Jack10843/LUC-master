...
class SACTrainer(TorchTrainer):
    ...
    def train_from_torch(self, batch, indices):
        ...
        # with torch.no_grad():
        #     actions_next, _, _, _, *_ = self.policy(next_obs, reparameterize=False)
        #     qs_pred_next = self.qfs(next_obs, actions_next)
        #     penalty = torch.clamp(qs_pred.detach().std(axis=0) - self.discount * qs_pred_next.std(axis=0), 0, None)
        # q_target = self.reward_scale * (rewards - 0.5 * penalty.detach()) + future_values
        ...
        if self.q_ood_reg > 0: # self.q_ood_eps = 0 for PBRL
            ...
            if self.q_ood_uncertainty_reg > 0:
                ...
                with torch.no_grad():
                    _,_,_, noised_next_obs,_ = self._get_noised_obs(next_obs, actions, self.q_ood_eps)
                    ood_actions_next, _, _, _, *_ = self.policy(noised_next_obs, reparameterize=False)
                    qs_pred_next = self.qfs(noised_next_obs, ood_actions_next)
                    penalty = torch.clamp(self.discount * qs_pred_next.std(axis=0) - ood_qs_pred.detach().std(axis=0), 0, None)

                ood_target = ood_qs_pred - self.q_ood_uncertainty_reg * ood_qs_pred.std(axis=0) - self.q_ood_uncertainty_reg * penalty
